...
if [[ ! $1 ]]; then
  echo "Usage: ./compose-up.sh <docker compose file> [<moqui directory like ..>] [<runtime image like eclipse-temurin:11-jdk>] [<.env file like .env>]"
  exit 1
fi
...
if ! [ -z "$ENV_FILE" ]; then
  docker compose -f $COMP_FILE --env-file $ENV_FILE -p moqui up -d
else
